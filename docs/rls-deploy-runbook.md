...
**상태: 미반영.** 이 브랜치(`feature/integration`)에 RLS 마이그레이션이 들어 있지만 운영 DB에는 아직 적용되지 않았습니다. 작성자에게 운영 DB 권한이 없어, 권한을 가진 사람이 이어받도록 절차를 남깁니다.

작성일 2026-08-28 · 갱신 2026-08-31(UI_renewal 통합으로 번호가 016~019로 바뀜).
...
| 파일                                          | 내용                                                                    |
| --------------------------------------------- | ----------------------------------------------------------------------- |
| `supabase/migrations/016_rls.sql`             | 전 테이블 RLS 정책 + 헬퍼 함수 + `is_master` 권한 상승 차단 트리거      |
| `supabase/migrations/017_grants.sql`          | 롤별 테이블 권한(GRANT). 정책과 별개의 관문이라 둘 다 필요              |
| `supabase/migrations/018_delete_user_fks.sql` | 탈퇴를 막던 외래키를 `ON DELETE SET NULL`로 변경 (조직 개편분 3개 포함) |
| `supabase/migrations/019_org_rls.sql`         | 조직 테이블(divisions/departments/…) RLS + 조회 범위를 부서까지 확장    |
...
정책이 반영하는 접근 모델은 `016_rls.sql` 상단 주석에 있습니다. 요약하면 — 본인 데이터는 본인이, 같은 팀의 근무/휴가/원격근무/출근계획은 팀원이 읽기, 팀 관리는 팀장, 마스터 계정은 전체, 서버 API 라우트는 service_role로 RLS 우회.
...
2. 어느 파일에도 `GRANT`문이 없었습니다. 운영은 대시보드가 권한을 만들어 줘서 동작 중이었습니다. → `017_grants.sql`로 명시했습니다.
...
- **마스터가 403을 받는다** → `service_role`에 테이블 권한이 없는 경우입니다. API 라우트의 `isMaster()`가 fail-closed라 false를 반환합니다. `017_grants.sql`이 막아주지만, 010을 건너뛰면 이 증상이 나옵니다.
...
- **anon 권한은 회수하지 않았습니다** — Supabase 대시보드 기본값은 `anon`에게도 테이블 ALL 권한을 줍니다. `017_grants.sql`은 anon에 권한을 주지 않을 뿐 기존 권한을 REVOKE하지 않으므로, **운영 DB의 anon 권한은 그대로 남습니다.** 009의 정책이 전부 `TO authenticated`라 실제 접근은 막히지만, 관문을 하나 더 두려면 REVOKE를 별건으로 검토하세요.
