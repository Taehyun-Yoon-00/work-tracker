...
import type { DepartmentApprover, Profile, TeamRole, UUID } from '../lib/types'
import {
  DivisionRow,
  DepartmentRow,
  isDivisionHead,
...
/** 조인해서 함께 가져오는 profiles 컬럼 (조직도에는 이름만 필요하다) */
type OrgJoinedProfile = Pick<Profile, 'name' | 'email' | 'is_master'> | null
type OrgTeamMemberRow = {
  team_id: UUID
  user_id: UUID
  role: TeamRole | null
  profiles: OrgJoinedProfile
}
type OrgDirectMemberRow = { department_id: UUID; user_id: UUID; profiles: OrgJoinedProfile }
type OrgApproverRow = Pick<
  DepartmentApprover,
  'id' | 'department_id' | 'user_id' | 'can_vacation' | 'can_remote' | 'can_holiday'
> & { profiles: OrgJoinedProfile }

/** 결재 위임 유형별 허용 여부 */
type ApproverChecks = Pick<DepartmentApprover, 'can_vacation' | 'can_remote' | 'can_holiday'>
...
export default function OrgPage() {
  ...
  const [modalChecks, setModalChecks] = useState<ApproverChecks>({
    can_vacation: true,
    can_remote: true,
    can_holiday: true,
  })
  ...
  const fetchAll = async () => {
    ...
    // 부서에 배정되지 않은 팀은 조직도에 걸 자리가 없다
    setTeams((teamRes.data ?? []).filter((t) => t.department_id))
    setTeamMembers(
      (tmRes.data ?? [])
    ...
    setApproverDelegates(
      (apRes.data ?? [])
        .filter((a) => !a.profiles?.is_master)
        .map((a) => ({
          id: a.id,
          department_id: a.department_id,
          user_id: a.user_id,
          name: displayName(a.profiles, '이름없음'),
  ...
  // checking이 끝난 뒤에만 화면이 그려지므로 이 시점의 user는 항상 있다.
  ...
  const assignableDepartments = (): DepartmentRow[] =>
    hasTopAccess
      ...
      : departments.filter((d) =>
          canManageDepartment(userId, d.id, departments, divisions, hasTopAccess)
        )
  ...
  const submitModal = async () => {
    ...
    if (modal.kind === 'createTeam') {
      ...
      const { error } = await supabase
        .from('teams')
        .insert({ name: modalInput.trim(), department_id: modal.departmentId, created_by: userId })
      if (error) {
  ...
  const departmentsForActor = (
    ...
    if (isDivisionHead(userId, division, hasTopAccess))
  ...
  function renderDivisionDetail(division: DivisionRow, mobile = false) {
    const canManageThisDivision = canManageDivision(userId, division.id, divisions, hasTopAccess)
                  ...
                  <div className="space-y-1">
                    ...
                    {teamMembersFlat.map((m) => (
                      ...
                      >
                        <span>{m.name}</span>
                        {m.role === 'admin' && (
                          <span className="text-[10px] text-blue-500">팀장</span>
                        )}
                      </div>
                    ))}
                    {!headEntry && directs.length === 0 && teamMembersFlat.length === 0 && (
                      <span className="text-[11px] text-gray-300 dark:text-zinc-600">
                        소속 인원 없음
                      </span>
                    )}
                  </div>
                            ...
                            {canManageThisDept &&
                              (movingKey === key ? (
                                <MoveForm
                                  ...
                                  teamsInDept={teamsInDept}
                                  onConfirm={() =>
                                    handleConfirmMove(d.user_id, null, department.id)
                                  }
                                  onCancel={closeMove}
                                />
                              ) : (
                                <>
                                  <button
                                    onClick={() => openMove(key, department.id, null)}
                                    className="text-xs text-blue-400 hover:underline"
                                  >
                                    이동
                                  </button>
                                  <button
                                    onClick={() => handleUnassignFromDept(department.id, d.user_id)}
                                    className="text-xs text-red-400 hover:underline"
                                  >
                                    내보내기
                                  </button>
                  ...
                  {canManageThisDivision &&
                    ...
                    (movingKey === headKey ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={() =>
                            handleUnassignFromDept(department.id, department.head_user_id as string)
                          }
                          className="text-xs text-red-400 hover:underline"
                        >
                          내보내기
                          ...
                          {canManageThisDept &&
                            ...
                            (movingKey === `team-${team.id}-${m.user_id}` ? (
                              ...
                              <>
                                <button
                                  onClick={() =>
                                    openMove(`team-${team.id}-${m.user_id}`, department.id, team.id)
                                  }
                                  className="text-xs text-blue-400 hover:underline"
                                >
                                  이동
                                ...
                                <button
                                  onClick={() => handleUnassignFromTeam(team.id, m.user_id)}
                                  className="text-xs text-red-400 hover:underline"
                                >
                                  내보내기
                                </button>
  ...
  return (
    <div
      className="grow bg-gray-50 dark:bg-zinc-900 p-2 sm:p-4 pb-6"
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      {(
                        ...
                        ] as { key: keyof ApproverChecks; label: string }[]
                      ).map(({ key, label }) => (
                        <label
                          key={key}
...
function MoveForm({
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1 max-w-full">
      ...
      <button onClick={onConfirm} className="text-green-500 font-medium shrink-0">
        확인
      </button>
      <button onClick={onCancel} className="text-gray-400 shrink-0">
        취소
      </button>
    </span>
  )
