...
export default function OrgPage() {
  ...
  // 구성원 행의 ⋮ 메뉴(이동/내보내기 선택지) 펼침 상태 — 한 번에 하나만 펼쳐진다.
  ...
  const fetchAll = async () => {
    ...
    setAllProfiles(
      (profRes.data || []).map((p: any) => ({
        id: p.id,
        name: p.name || p.email?.split('@')[0] || '이름없음',
      }))
    )
  ...
  const profileName = (id: string | null) =>
    id ? allProfiles.find((p) => p.id === id)?.name || '알 수 없음' : null
  ...
  const memberIdsOfDivision = (divisionId: string): Set<string> => {
    const ids = new Set<string>()
    departmentsOfDivision(divisionId).forEach((d) =>
      memberIdsOfDept(d.id).forEach((id) => ids.add(id))
    )
  ...
  const submitModal = async () => {
    ...
    if (modal.kind === 'createDepartment') {
      ...
      const { error } = await supabase
        .from('departments')
        .insert({ division_id: modal.divisionId, name: modalInput.trim() })
      if (error) {
        setMessage('부서 생성 실패: ' + error.message)
        return
      }
    }
    ...
    if (modal.kind === 'createTeam') {
      ...
      const { error } = await supabase
        .from('teams')
        .insert({ name: modalInput.trim(), department_id: modal.departmentId, created_by: user.id })
      if (error) {
    ...
    if (modal.kind === 'assignDivisionHead') {
      ...
      if (error) {
        setMessage('부문장 지정 실패: ' + error.message)
        return
      }
    }
    ...
    if (modal.kind === 'assignDepartmentHead') {
      ...
      const previousHeadId =
        departments.find((d) => d.id === modal.departmentId)?.head_user_id || null
      ...
      if (error) {
        setMessage('부서장 지정 실패: ' + error.message)
        return
      }
      ...
      if (modalUserId) {
        const alreadyBelongsToDept =
          directMembers.some(
            (m) => m.department_id === modal.departmentId && m.user_id === modalUserId
          ) ||
          teamsInDept(modal.departmentId).some((t) =>
            membersOfTeam(t.id).some((m) => m.user_id === modalUserId)
          )
        if (!alreadyBelongsToDept) {
          await supabase
            .from('department_memberships')
            .insert({ department_id: modal.departmentId, user_id: modalUserId })
        }
      }
    ...
    if (modal.kind === 'assignTeamHead') {
      if (modalUserId) {
        ...
        const currentLeads = teamMembers.filter(
          (m) => m.team_id === modal.teamId && m.role === 'admin'
        )
  ...
  const performAssignTeamLead = async (teamId: string, departmentId: string, userId: string) => {
    const currentLeads = teamMembers.filter(
      (m) => m.team_id === teamId && m.role === 'admin' && m.user_id !== userId
    )
    for (const lead of currentLeads) {
      await supabase
        .from('team_members')
    ...
    const oldTeamMembership = teamMembers.find(
      (m) =>
        m.user_id === userId &&
        m.team_id !== teamId &&
        teams.find((t) => t.id === m.team_id)?.department_id === departmentId
    )
    if (oldTeamMembership) {
      await supabase
        .from('team_members')
        .delete()
        .eq('team_id', oldTeamMembership.team_id)
        .eq('user_id', userId)
    }
    await supabase
      .from('department_memberships')
    ...
    if (existing) {
      ...
      await supabase
        .from('team_members')
        .insert({ team_id: teamId, user_id: userId, role: 'admin' })
    }
  }
  ...
  // 이 팀의 현재 팀장(1명) 조회
  const teamLeadOf = (teamId: string) =>
    teamMembers.find((m) => m.team_id === teamId && m.role === 'admin') || null

  // ---------- 구성원 이동 ----------
  const departmentsForActor = (
    division: DivisionRow,
    departmentIdHint: string
  ): DepartmentRow[] => {
    if (hasTopAccess) return departments
    if (isDivisionHead(user.id, division, hasTopAccess))
      return departments.filter((d) => d.division_id === division.id)
  ...
  const handleConfirmAssign = async (userId: string) => {
    ...
    if (assignTargetTeam) {
      const { error } = await supabase
        .from('team_members')
        .insert({ team_id: assignTargetTeam, user_id: userId, role: 'member' })
      if (error) {
        setMessage('배정 실패: ' + error.message)
        return
      }
    } else {
      const { error } = await supabase
        .from('department_memberships')
        .insert({ department_id: assignTargetDept, user_id: userId })
      if (error) {
        setMessage('배정 실패: ' + error.message)
        return
      }
  ...
  const confirmUnassignFromTeam = (teamId: string, userId: string) => {
    setPendingConfirm({
      title: '이 사람을 이 팀에서 내보낼까요?',
      confirmLabel: '내보내기',
      tone: 'danger',
      onConfirm: () => {
        setPendingConfirm(null)
        handleUnassignFromTeam(teamId, userId)
      },
    })
  }
  ...
  const confirmUnassignFromDept = (departmentId: string, userId: string) => {
    if (!departmentId) return
    setPendingConfirm({
      title: '이 사람을 이 부서에서 내보낼까요?',
      confirmLabel: '내보내기',
      tone: 'danger',
      onConfirm: () => {
        setPendingConfirm(null)
        handleUnassignFromDept(departmentId, userId)
      },
    })
  }
  ...
  const handleConfirmMove = async (
    ...
    if (moveTargetTeam) {
      await supabase
        .from('team_members')
        .insert({ team_id: moveTargetTeam, user_id: userId, role: 'member' })
    } else {
      await supabase
        .from('department_memberships')
        .insert({ department_id: moveTargetDept, user_id: userId })
    }
    closeMove()
    fetchAll()
  }
  ...
  const handleDeleteDivision = async (division: DivisionRow) => {
    ...
    if (error) {
      setMessage('부문 삭제 실패: ' + error.message)
      return
    }
  ...
  const handleDeleteTeam = async (team: TeamRow) => {
    ...
    if (error) {
      setMessage('팀 삭제 실패: ' + error.message)
      return
    }
    setOpenStructureItem((prev) => (prev === `team-${team.id}` ? null : prev))
    setSelected((prev) =>
      prev?.type === 'team' && prev.id === team.id
        ? { type: 'department', id: team.department_id }
        : prev
    )
    fetchAll()
  }
  ...
  const handleDeleteDepartment = async (department: DepartmentRow) => {
    ...
    if (error) {
      setMessage('부서 삭제 실패: ' + error.message)
      return
    }
    setSelected((prev) =>
      prev?.type === 'department' && prev.id === department.id
  ...
  const finishReorder = async () => {
    ...
    try {
      if (reorder.kind === 'divisions') {
        ...
        await Promise.all(
          group.map((d, idx) =>
            supabase.from('departments').update({ display_order: idx }).eq('id', d.id)
          )
        )
        ...
        await Promise.all(
          group.map((t, idx) =>
            supabase.from('teams').update({ display_order: idx }).eq('id', t.id)
          )
        )
        ...
        const tasks = directGroup.map((m, idx) =>
          supabase
            ...
            .eq('department_id', reorder.departmentId)
            .eq('user_id', m.user_id)
        )
        const teamIds = teams
          .filter((t) => t.department_id === reorder.departmentId)
          .map((t) => t.id)
  ...
  const selectedDivision =
    selected?.type === 'division' ? divisions.find((d) => d.id === selected.id) || null : null
  ...
  const parentDivisionOfSelectedDept = selectedDepartment
    ...
    : null
  const selectedTeam =
    selected?.type === 'team' ? teams.find((t) => t.id === selected.id) || null : null
            ...
            >
              ...
              {myDepts.length === 0 && leadTeams.length === 0 ? (
                ...
                myDepts.length > 0 ? (
                  ...
                  <span className="inline-flex flex-wrap items-center gap-1 max-w-full">
                    ...
                    <button
                      ...
                      className="text-[11px] text-green-500 font-medium shrink-0 px-1 py-0.5"
                    >
                      확인
                    </button>
                    <button
                      onClick={closeAssign}
                      className="text-[11px] text-gray-400 shrink-0 px-1 py-0.5"
                    >
                      취소
                    </button>
                  </span>
                ...
                <button
                  ...
                  className="text-xs text-blue-500 hover:underline font-medium shrink-0"
                >
                  + 배정
                </button>
              )}
        ...
        <div className="flex items-start justify-between mb-1 gap-2">
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-400 dark:text-zinc-500 mt-0.5">
              ...
              {headcountOfDivision(division.id)}명
            </p>
          </div>
            ...
            {(hasTopAccess || canManageThisDivision) && (
              <>
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    setMenuOpenId(menuOpenId === `div-${division.id}` ? null : `div-${division.id}`)
                  }}
                ...
                {menuOpenId === `div-${division.id}` && (
                  <div
                    ...
                    className="absolute right-0 top-9 bg-white dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 rounded-lg shadow-lg py-1 w-36 z-10"
                  >
                    {hasTopAccess && (
                      <button
                        onClick={() => {
                          setMenuOpenId(null)
                          openModal({ kind: 'assignDivisionHead', divisionId: division.id })
                        }}
          ...
          <div>
            ...
            <div className={`space-y-2 ${dimClass(!reorder || isReorderingDepts)}`}>
              <DragList
                ...
                onReorder={(next) =>
                  setDepartments((prev) =>
                    replaceGroup(prev, (d) => d.division_id === division.id, next)
                  )
                }
                    ...
                    <div className="w-full flex items-center justify-between bg-gray-50 dark:bg-zinc-900/40 rounded-lg px-4 py-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-400 dark:text-zinc-500 mt-0.5">
                          {profileName(department.head_user_id) || '부서장 공석'} ·{' '}
        ...
        {activeTab === 'members' && (
          <div className={`space-y-4 ${dimClass(!reorder)}`}>
            {division.head_user_id && (
              <div>
                ...
                <div className="relative flex items-center flex-wrap justify-between gap-y-1 gap-x-2 py-1">
                  ...
                  <MemberActionMenu
                    rowKey={headKey}
                    openKey={openActionKey}
                    setOpenKey={setOpenActionKey}
                    movingKey={movingKey}
                    canMove={hasTopAccess}
                    canRemove={
                      hasTopAccess &&
                    ...
                    onMoveClick={() =>
                      openMove(
                        headKey,
                        directMembers.find((m) => m.user_id === division.head_user_id)
                          ?.department_id ||
                          deptList[0]?.id ||
                          '',
                        null
            ...
            {deptList.map((department) => {
              ...
              const directs = directOfDept(department.id).filter(
                (m) => m.user_id !== department.head_user_id
              )
              ...
              const headEntry = department.head_user_id
                ? directOfDept(department.id).find((m) => m.user_id === department.head_user_id)
                : null
              const canManageThisDeptRow = canManageDepartment(
                user.id,
                department.id,
                departments,
                divisions,
                  ...
                  <div className="divide-y divide-gray-50 dark:divide-zinc-900/60">
                    {headEntry && (
                      <div className="relative flex items-center flex-wrap justify-between gap-y-1 gap-x-2 py-1">
                        ...
                        <MemberActionMenu
                          rowKey={`div-direct-${department.id}-${headEntry.user_id}`}
                          openKey={openActionKey}
                          setOpenKey={setOpenActionKey}
                          movingKey={movingKey}
                          canMove={canManageThisDeptRow}
                          canRemove={canManageThisDeptRow}
                          onMoveClick={() =>
                            openMove(
                              `div-direct-${department.id}-${headEntry.user_id}`,
                              department.id,
                              null
                            )
                          }
                          onRemoveClick={() =>
                            confirmUnassignFromDept(department.id, headEntry.user_id)
                          }
                          moveForm={
                            <MoveForm
                              ...
                              teamsInDept={teamsInDept}
                              onConfirm={() =>
                                handleConfirmMove(headEntry.user_id, null, department.id)
                              }
                              onCancel={closeMove}
                            />
                          }
                    ...
                    {directs.map((m) => (
                      <div key={m.user_id} className="relative flex items-center flex-wrap justify-between gap-y-1 gap-x-2 py-1">
                        ...
                        <MemberActionMenu
                          rowKey={`div-direct-${department.id}-${m.user_id}`}
                          openKey={openActionKey}
                          setOpenKey={setOpenActionKey}
                          movingKey={movingKey}
                          canMove={canManageThisDeptRow}
                          canRemove={canManageThisDeptRow}
                          onMoveClick={() =>
                            openMove(`div-direct-${department.id}-${m.user_id}`, department.id, null)
                          }
                          onRemoveClick={() => confirmUnassignFromDept(department.id, m.user_id)}
                          moveForm={
                            <MoveForm
                              departmentsForActor={departmentsForActor(division, department.id)}
                              moveTargetDept={moveTargetDept}
                              ...
                              setMoveTargetTeam={setMoveTargetTeam}
                              teamsInDept={teamsInDept}
                              onConfirm={() => handleConfirmMove(m.user_id, null, department.id)}
                              onCancel={closeMove}
                            />
                          }
                        />
                    ...
                    {teamMembersFlat.map((m) => (
                      ...
                      >
                        ...
                        <MemberActionMenu
                          rowKey={`div-team-${m.team_id}-${m.user_id}`}
                          openKey={openActionKey}
                          setOpenKey={setOpenActionKey}
                          movingKey={movingKey}
                          canMove={canManageThisDeptRow}
                          canRemove={canManageThisDeptRow}
                          onMoveClick={() =>
                            openMove(`div-team-${m.team_id}-${m.user_id}`, department.id, m.team_id)
                          }
                          onRemoveClick={() => confirmUnassignFromTeam(m.team_id, m.user_id)}
                          moveForm={
                            <MoveForm
                              departmentsForActor={departmentsForActor(division, department.id)}
                              moveTargetDept={moveTargetDept}
                              ...
                              teamsInDept={teamsInDept}
                              onConfirm={() =>
                                handleConfirmMove(m.user_id, m.team_id, department.id)
                              }
                              onCancel={closeMove}
                            />
                          }
                    ...
                    {!headEntry && directs.length === 0 && teamMembersFlat.length === 0 && (
                      <p className="text-[11px] text-gray-300 dark:text-zinc-600 py-2">
                        소속 인원 없음
                      </p>
                    )}
                  </div>
  ...
  function renderDepartmentDetail(
    ...
    const isReorderingMembers =
      reorder?.kind === 'deptMembers' && reorder.departmentId === department.id
      ...
      <div>
        {!mobile && (
          <button
            ...
            className={`text-xs text-gray-300 dark:text-zinc-600 mb-1 hover:underline ${dimClass(!reorder)}`}
          >
            {division.name}
          </button>
        )}
              ...
              <>
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    setMenuOpenId(
                      menuOpenId === `dept-${department.id}` ? null : `dept-${department.id}`
                    )
                  }}
                  className="text-gray-400 dark:text-zinc-500 px-1.5 py-1.5"
                >
                  ...
                  >
                    {canManageThisDivision && (
                      <button
                        onClick={() => {
                          setMenuOpenId(null)
                          openModal({ kind: 'assignDepartmentHead', departmentId: department.id })
                        }}
                        className="w-full text-left px-3 py-1.5 text-xs text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-700"
                      >
                        부서장 변경
                    ...
                    {canManageThisDept &&
                      directs.length +
                        deptTeams.reduce((acc, t) => acc + membersOfTeam(t.id).length, 0) >
                        1 && (
                        <button
          ...
          <div>
            ...
            <div className={`mb-2 ${dimClass(!isReorderingTeams)}`}>
              ...
              >
                ...
                <span className="text-gray-300 dark:text-zinc-600 shrink-0 ml-2">
                ...
                <div className="mt-1.5 px-4 py-3 bg-gray-50/60 dark:bg-zinc-900/20 rounded-lg">
                  {directs.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                      {/* 이동/내보내기는 "구성원" 탭에서만 할 수 있다 — 팀 구조 탭의 부서 직속
                          펼침 목록은 보기 전용이다. */}
                      {directs.map((d) => {
                        ...
                        return (
                          <div
                            key={key}
                            className="flex items-center gap-2 flex-wrap text-sm text-gray-600 dark:text-zinc-300"
                          >
                            <span>{d.name}</span>
                            {d.user_id === department.head_user_id && (
            ...
            <div className="space-y-2">
              <DragList
                ...
                onReorder={(next) =>
                  setTeams((prev) =>
                    replaceGroup(prev, (t) => t.department_id === department.id, next)
                  )
                }
                renderItem={(team) => {
          ...
          <div className={`space-y-4 ${dimClass(!reorder || isReorderingMembers)}`}>
            ...
            <div>
              {/*
                ...
                이동/내보내기는 이름과 같은 행 오른쪽 끝의 ⋮ 버튼을 눌렀을 때만 펼쳐진다.
              */}
              {(() => {
                      ...
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span
                          className={`text-sm truncate ${
                            isHead ? 'font-medium dark:text-white' : 'text-gray-600 dark:text-zinc-300'
                          }`}
                        >
                          {d.name}
                        </span>
                        {isHead && <span className="text-[10px] text-blue-500 shrink-0">부서장</span>}
                      </div>
                      {!isReorderingMembers && (
                        <MemberActionMenu
                          rowKey={rowKey}
                          ...
                          setOpenKey={setOpenActionKey}
                          movingKey={movingKey}
                          canMove={canManageRow}
                          canRemove={canManageRow}
                          onMoveClick={() => openMove(rowKey, department.id, null)}
                          onRemoveClick={() => confirmUnassignFromDept(department.id, d.user_id)}
                          moveForm={
                            <MoveForm
                              departmentsForActor={departmentsForActor(division, department.id)}
                              moveTargetDept={moveTargetDept}
                ...
                <div key={team.id}>
                  ...
                  <div className="divide-y divide-gray-50 dark:divide-zinc-900/60">
                    <DragList
                      ...
                      onReorder={(next) =>
                        setTeamMembers((prev) =>
                          replaceGroup(prev, (m) => m.team_id === team.id, next)
                        )
                      }
                      renderItem={(m) => {
                        const rowKey = `team-${team.id}-${m.user_id}`
                        return (
                          <div className="relative flex items-center flex-wrap justify-between gap-y-1 gap-x-2 py-1">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="text-sm text-gray-600 dark:text-zinc-300 truncate">
                            ...
                            {!isReorderingMembers && (
                              <MemberActionMenu
                                ...
                                setOpenKey={setOpenActionKey}
                                movingKey={movingKey}
                                canMove={canManageThisDept}
                                canRemove={canManageThisDept}
                                onMoveClick={() => openMove(rowKey, department.id, team.id)}
                                onRemoveClick={() => confirmUnassignFromTeam(team.id, m.user_id)}
                                moveForm={
                                  <MoveForm
                                    departmentsForActor={departmentsForActor(division, department.id)}
                                    moveTargetDept={moveTargetDept}
                                    ...
                                    teamsInDept={teamsInDept}
                                    onConfirm={() =>
                                      handleConfirmMove(m.user_id, team.id, department.id)
                                    }
                                    onCancel={closeMove}
                                  />
                                }
    ...
    return (
      <div>
        {!mobile && (
          <button
            onClick={() => selectDepartment(department.id)}
            className="text-xs text-gray-300 dark:text-zinc-600 mb-1 hover:underline"
          >
            {department.name}
          </button>
        )}
            ...
            {canManageThisDept && (
              <>
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    setMenuOpenId(menuOpenId === `team-${team.id}` ? null : `team-${team.id}`)
                  }}
                  className="text-gray-400 dark:text-zinc-500 px-1.5 py-1.5"
                >
                  ⋮
                ...
                {menuOpenId === `team-${team.id}` && (
                  <div
                    onClick={(e) => e.stopPropagation()}
                    className="absolute right-0 top-9 bg-white dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 rounded-lg shadow-lg py-1 w-32 z-10"
                  >
                    <button
                      onClick={() => {
                        setMenuOpenId(null)
        ...
        {activeTab !== 'permissions' && (
          <div className="divide-y divide-gray-50 dark:divide-zinc-900/60">
            ...
            {members.map((m) => {
              ...
              return (
                <div key={key} className="relative flex items-center flex-wrap justify-between gap-y-1 gap-x-2 py-1">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className="text-sm text-gray-600 dark:text-zinc-300 truncate">{m.name}</span>
                    {m.role === 'admin' && (
                      <span className="text-[10px] text-blue-500 shrink-0">팀장</span>
                    )}
                  </div>
                  {canManageThisTeam && (
                    <MemberActionMenu
                      ...
                      canMove={canManageThisDept}
                      canRemove={true}
                      onMoveClick={() => openMove(key, department.id, team.id)}
                      onRemoveClick={() => confirmUnassignFromTeam(team.id, m.user_id)}
                      moveForm={
                        <MoveForm
                          departmentsForActor={departmentsForActor(division, department.id)}
  ...
  return (
    <div
      className="min-h-screen bg-gray-50 dark:bg-zinc-900 p-2 sm:p-4 pb-28"
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden py-2">
              ...
              {reorder?.kind === 'divisions' ? (
                ...
                const deptList = departmentsOfDivision(division.id).filter(
                  (d) => !searchTerm || departmentMatchesSearch(d)
                )
                return (
                  <div key={division.id}>
                    <button
                    ...
                    {isOpen && (
                      <div className="pb-1.5 ml-6 pl-3 border-l-2 border-gray-100 dark:border-zinc-700 space-y-0.5 min-w-0 overflow-hidden">
                        {deptList.length === 0 && (
                          <p className="pr-3 py-1 text-[11px] text-gray-300 dark:text-zinc-600">
                            등록된 부서가 없어요.
                          </p>
                        )}
                        {deptList.map((department) => {
                          const isDeptSelected =
                            selected?.type === 'department' && selected.id === department.id
                          const deptTeams = teamsInDept(department.id).filter(
                            (t) =>
                              !searchTerm || matchesTerm(t.name) || matchesTerm(department.name)
                          )
                          const isDeptOpen =
                            deptTeams.length > 0 &&
                            (expandedDept.has(department.id) || !!searchTerm)
                          return (
                            <div key={department.id}>
                              <button
                                ...
                                <div className="ml-3 pl-3 border-l-2 border-gray-50 dark:border-zinc-800 space-y-0.5 min-w-0 overflow-hidden">
                                  {deptTeams.map((team) => {
                                    const isTeamSelected =
                                      selected?.type === 'team' && selected.id === team.id
                                    return (
                                      <button
                                        key={team.id}
                                        onClick={() => selectTeam(team.id)}
                                      ...
                                      >
                                        ...
                                        <span className="text-gray-300 dark:text-zinc-600 shrink-0">
                                          {membersOfTeam(team.id).length}
                                        </span>
                                      </button>
                                    )
                                  })}
          ...
          >
            ...
            {selectedDepartment &&
              parentDivisionOfSelectedDept &&
              renderDepartmentDetail(selectedDepartment, parentDivisionOfSelectedDept)}
            {selectedTeam &&
              parentDeptOfSelectedTeam &&
              parentDivOfSelectedTeamDept &&
              renderTeamDetail(selectedTeam, parentDeptOfSelectedTeam, parentDivOfSelectedTeamDept)}
            {!selected && (
              <p className="text-sm text-gray-400 dark:text-zinc-500">
                왼쪽에서 부문, 부서 또는 미지정 인원을 선택해주세요.
              </p>
            )}
          </main>
        </div>
        ...
        <div className="md:hidden" onClick={(e) => e.stopPropagation()}>
          ...
          >
            ...
            <option value="__unassigned__">
              미지정 인원 {unassignedProfiles.length > 0 ? `(${unassignedProfiles.length})` : ''}
            </option>
          </select>
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              {(modal.kind === 'createDivision' ||
                modal.kind === 'createDepartment' ||
                modal.kind === 'createTeam') && (
                <input
                  autoFocus
                  value={modalInput}
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      {[
                        ...
                        <label
                          key={key}
                          className="flex items-center gap-1.5 text-sm dark:text-zinc-200"
                        >
                          <input
                            type="checkbox"
                            checked={(modalChecks as any)[key]}
                            onChange={(e) =>
                              setModalChecks((s) => ({ ...s, [key]: e.target.checked }))
                            }
                          />
                          {label}
                        </label>
            ...
            <div className="flex gap-3 mt-7">
              <button
                onClick={closeModal}
                className="flex-1 bg-gray-100 dark:bg-zinc-700 text-gray-600 dark:text-zinc-300 py-2.5 rounded-lg text-sm font-medium"
              >
                취소
              </button>
              <button
                onClick={submitModal}
                className="flex-1 bg-blue-500 text-white py-2.5 rounded-lg text-sm font-medium"
              >
                확인
              </button>
            </div>
...
/**
 * 구성원 행 오른쪽 끝의 ⋮ 버튼. 누르면 이동/내보내기 선택지가 펼쳐지고,
 * "이동"을 고르면 movingKey가 이 행으로 바뀌면서 MoveForm으로 전환된다.
 * canMove/canRemove가 모두 false면 아무것도 렌더링하지 않는다(권한 없는 행은 조용히 생략).
 */
function MemberActionMenu({
  ...
  if (movingKey === rowKey) {
    return <div className="w-full flex justify-end mt-1">{moveForm}</div>
  }
  if (!canMove && !canRemove) return null
  const isOpen = openKey === rowKey
  return (
    <div className="relative shrink-0">
      <button
        onClick={() => setOpenKey(isOpen ? null : rowKey)}
        className="w-6 h-6 flex items-center justify-center rounded text-gray-400 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-600 dark:hover:text-zinc-300 leading-none text-base"
        aria-label="더보기"
      >
        ⋮
      </button>
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 min-w-[92px] bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg shadow-lg py-1 flex flex-col">
            {canMove && (
              <button
                onClick={() => {
                  setOpenKey(null)
                ...
                className="px-3 py-1.5 text-xs text-left text-blue-500 hover:bg-gray-50 dark:hover:bg-zinc-700"
              >
                이동
              </button>
            )}
            {canRemove && (
              <button
                ...
                className="px-3 py-1.5 text-xs text-left text-red-500 hover:bg-gray-50 dark:hover:bg-zinc-700"
              >
                내보내기
              </button>
            )}
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1 max-w-full">
      <select
        ...
        onChange={(e) => {
          setMoveTargetDept(e.target.value)
          setMoveTargetTeam('')
        }}
        className="border rounded-lg px-1.5 py-0.5 text-[11px] dark:bg-zinc-700 dark:border-zinc-600 dark:text-zinc-200 max-w-[40vw] sm:max-w-[160px]"
      >
      ...
      <button onClick={onCancel} className="text-[11px] text-gray-400 shrink-0 px-1 py-0.5">
        취소
      </button>
    </span>
  )
        ...
        return (
          ...
          >
            <div className="flex items-center gap-1.5">
              <span className="shrink-0 text-gray-300 dark:text-zinc-600 select-none" aria-hidden>
                ⠿
              </span>
