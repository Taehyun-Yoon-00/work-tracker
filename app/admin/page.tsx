...
export default function AdminPage() {
  ...
  // window.confirm 대신 쓰는 확인창 상태 (approval/page.tsx와 같은 패턴)
  ...
  const handleDelete = async (profile: any) => {
    ...
    // work_logs/vacations/remote_works/commute_plans/team_members/team_requests는
    // profiles(id)를 ON DELETE CASCADE로 참조하므로 /api/admin/delete-user가
    // profiles를 지우면 함께 정리된다. 여기서 미리 하나씩 지울 필요가 없다.
  ...
  const confirmDelete = (profile: any) => {
    ...
    setPendingConfirm({
      title: `"${profile.name || profile.email}" 회원을 강제 탈퇴시킬까요?`,
  ...
  const handleToggleGeneralAdmin = async (profile: any) => {
    ...
    if (isCurrentlyGeneralAdmin) {
      ...
      if (error) {
        setMessage('총괄 관리자 해제 실패: ' + error.message)
        return
      }
    } else {
      ...
      if (error) {
        setMessage('총괄 관리자 지정 실패: ' + error.message)
        return
      }
    }
    fetchProfiles()
  ...
  const confirmResetPassword = (profile: any) => {
    setPendingConfirm({
      title: `"${profile.name || profile.email}" 의 비밀번호를 초기화할까요?`,
      confirmLabel: '초기화',
      onConfirm: () => {
        setPendingConfirm(null)
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap justify-end">
                ...
                {!profile.is_master && (
                  <>
                    <button
                      onClick={() => confirmResetPassword(profile)}
