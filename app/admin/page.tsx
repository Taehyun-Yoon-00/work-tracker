...
import type { Profile, SubstituteHoliday, UUID } from '@/app/lib/types'
...
export default function AdminPage() {
  ...
  // 총괄 관리자는 is_master와 별개 트랙이라 별도 테이블에서 온다 (014)
  const [generalAdminIds, setGeneralAdminIds] = useState<Set<UUID>>(new Set())
  ...
  const fetchProfiles = async () => {
    setLoadFailed(false)
    const [{ data, error }, { data: generalAdminRows }] = await Promise.all([
      supabase
        .from('profiles')
    ...
    setGeneralAdminIds(new Set((generalAdminRows ?? []).map((r) => r.user_id)))
  ...
  const handleToggleGeneralAdmin = async (profile: AdminProfile) => {
    const { error } = generalAdminIds.has(profile.id)
      ? await supabase.from('general_admins').delete().eq('user_id', profile.id)
      : await supabase.from('general_admins').insert({ user_id: profile.id, created_by: user?.id })
    if (error) {
      setMessage('총괄 관리자 변경 실패: ' + error.message)
      return
    }
    fetchProfiles()
  }
  ...
  const handleCreateTestUser = async () => {
    ...
    if (!res.ok || data.error) {
      ...
      setMessage(`테스트 계정 생성 완료 — ${data.email} / 임시 비밀번호: ${data.password}`)
      setTestEmail('')
      setTestName('')
      fetchProfiles()
    }
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {generalAdminIds.has(profile.id) && (
                      <span className="text-[10px] bg-indigo-100 dark:bg-indigo-950 text-indigo-500 dark:text-indigo-300 px-2 py-0.5 rounded-full">
                        총괄 관리자
                      </span>
                    )}
                  </div>
                ...
                <div className="flex gap-2 flex-wrap justify-end">
                  ...
                  <button
                    ...
                    className={`text-xs px-2 py-1 rounded-lg ${
                      generalAdminIds.has(profile.id)
                        ? 'bg-gray-100 dark:bg-zinc-700 text-gray-600 dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-zinc-600'
                        : 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/50'
                    }`}
                  >
