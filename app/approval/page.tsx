...
export default function ApprovalPage() {
    ...
    const handleApprove = async (requestId: string, status: string) => {
        await supabase.from('approval_requests').update({ status }).eq('id', requestId)
        ...
        <div className="min-h-screen bg-gray-50 p-2 sm:p-4 pb-28">
            <div className="max-w-2xl mx-auto">
                ...
                <div className="flex bg-gray-100 rounded-lg p-0.5 mb-4">
                    {[
                        ...
                        <button key={value} onClick={() => setFilterStatus(value)}
                            className={`flex-1 text-xs py-1.5 rounded-md transition ${filterStatus === value ? 'bg-white shadow text-blue-500 font-semibold' : 'text-gray-500'
                                }`}>
                            {label}
                        </button>
                    ))}
                                ...
                                <div key={req.id} onClick={() => setSelectedRequest(req)}
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            {req.memo && <p className="text-xs text-gray-400">사유: {req.memo}</p>}
                                        </div>
                ...
                {selectedRequest && (
                    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                        <div className="bg-white rounded-xl p-6 w-full max-w-sm">
                            <h3 className="font-semibold mb-3">결재 처리</h3>
                            <div className="mb-4 text-sm text-gray-600 space-y-2">
                                ...
                                {selectedRequest.memo && (
                                    <p><span className="font-medium">사유:</span> {selectedRequest.memo}</p>
                                )}
                            </div>
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => setSelectedRequest(null)}
                                    className="flex-1 bg-gray-100 text-gray-600 py-2 rounded-lg text-sm">닫기</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {showRequestModal && (
                    ...
                    >
                        <div className="bg-white rounded-2xl w-full max-w-md max-h-[90dvh] flex flex-col">

                            {/* 헤더 */}
                            <div className="flex justify-between items-center p-4 sm:p-6 pb-3 border-b">
                                <h3 className="font-semibold">결재 요청</h3>
                                <button onClick={resetModal} className="text-gray-400 text-lg">✕</button>
                            </div>
                            ...
                            <div className="overflow-y-auto flex-1 p-4 sm:p-6">
                                ...
                                {step === 1 && (
                                    <div>
                                        <p className="text-sm text-gray-500 mb-3">요청 유형을 선택해주세요</p>
                                        <div className="flex gap-2">
                                            <button onClick={() => { setRequestType('vacation'); setStep(2) }}
                                                className="flex-1 py-3 border-2 rounded-xl text-sm font-medium hover:border-blue-500 hover:text-blue-500 transition">
                                                🌴 휴가
                                            </button>
                                ...
                                {step === 2 && (
                                    <div>
                                        <button onClick={() => setStep(1)} className="text-xs text-gray-400 mb-3">← 뒤로</button>

                                        {/* 팀 */}
                                        <div className="mb-3">
                                            <label className="text-sm text-gray-500">팀 선택</label>
                                        ...
                                        {/* 결재권자 */}
                                        <div className="mb-4">
                                            <label className="text-sm text-gray-500">결재권자</label>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex justify-between items-center mb-2">
                                                <label className="text-sm text-gray-500">날짜 선택</label>
                                                <button
                                            ...
                                            {dateGroups.length === 0 && (
                                                <p className="text-xs text-gray-400">날짜 추가 버튼을 눌러주세요</p>
                                            )}

                                            {dateGroups.map((group, index) => (
                                                <div key={index} className="mb-4 p-3 bg-gray-50 rounded-xl">
                                                    <div className="flex justify-between items-center mb-2">
                                                        <span className="text-xs text-gray-500">{index + 1}번째 그룹</span>
                                                        ...
                                                        <div className="flex gap-1 mt-3">
                                                            {[
                                                                ...
                                                                <button key={value}
                                                                    ...
                                                                    className={`flex-1 py-1.5 rounded-lg text-xs border ${group.vacationType === value ? 'bg-orange-500 text-white' : 'bg-white'
                                                                        }`}>
                                                                    {label}
                                                                </button>
                                                            ))}
                                        ...
                                        {requestType === 'vacation' && (
                                            <div className="mb-4">
                                                <label className="text-sm text-gray-500">휴가 사유</label>
